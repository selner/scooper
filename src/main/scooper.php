...
function __doRun__()
{
    ...
    if (isOptionEqualValue('crunchbase_api_url_given')) {
...
function __runCompanyLookups__()
{
    ...
    try {
        $arrInputCSVData = array('data_type' => null, 'data_rows' => null, 'lookup_type' => C_LOOKUP_MODE_SINGLE);

        if (isOptionEqualValue('lookup_name_given')) {
        ...
        if (!isset($GLOBALS['OPTS']['exclude_quantcast'])) $GLOBALS['OPTS']['exclude_quantcast'] = false;
        ...
        if (!isset($GLOBALS['OPTS']['exclude_angellist'])) $GLOBALS['OPTS']['exclude_angellist'] = false;
        ...
        if (!isset($GLOBALS['OPTS']['exclude_crunchbase'])) $GLOBALS['OPTS']['exclude_crunchbase'] = false;
        $pluginCrunchbase = new CrunchbasePluginClass($GLOBALS['OPTS']['exclude_crunchbase']);
        $arrAllPluginColumnsForRecords = \Scooper\my_merge_add_new_keys($arrAllPluginColumnsForRecords, $pluginCrunchbase->getAllColumns());

        if (!isset($GLOBALS['OPTS']['exclude_moz'])) $GLOBALS['OPTS']['exclude_moz'] = false;
        ...
        while ($ncurRecordIndex < count($arrAllRecordsProcessed)) {
            ...
            if ($ncurRecordIndex % C__RECORD_CHUNK_SIZE__ == 0) {
